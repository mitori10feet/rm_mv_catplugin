 ...
 * 插件指令:
 * 
 * MapZoom center 73 4 120  => 中心點座標 73 , 4, 持續時間 120
 * MapZoom center event 1 n 120  => 中心點座標為事件 1  持續時間 120 
 * MapZoom center event 0 n 120  => 中心點座標回到玩家 持續時間 120
 * 
 *
 * 
 * 
...
(function ($) {
  ...
  Game_Map.prototype.initialize = function() {
    ...
    this._zoom = this._zoom || new PIXI.Point($.Param.defaulztZoom, $.Param.defaultZoom);
    ...
    this._lastFocusCenter = null;

    this._fakePoint = new Game_PointEvent(this._mapId , 0);
  ...
  Game_Map.prototype.update = function () {
    ...
    this._fakePoint.update();
  	if (this._zoomDuration > this._zoomTime) {    
        ...
        this._lastFocusCenter = this._zoomCenter;        
    }
  ...
  Game_Map.prototype.setZoomCenter = function(a, b, d) {
    if (b === "n") b = null;
    
  	if (b) {
        this._lastFocusCenter = this._zoomCenter ? this._zoomCenter : $gamePlayer;

        this._fakePoint.locate(Number(a), Number(b));
        this._lastZoomSpeed = d || 60;

        this._zoomCenter = this._fakePoint;
        log(this._zoomCenter && this._lastFocusCenter);
        ...
        this._lastZoomSpeed = d || 60;
  		...
  		this._zoomCenter = $gamePlayer;

  	}
  };

  Game_Map.prototype.setDurationForView = function (duration){
        this._zoomDuration = duration;
  }
  ...
  Game_Map.prototype.onZoomChange = function() {
    if ( this._zoomCenter){
        ...
        x = this._zoomCenter.screenX() * this._zoom.x;
        y = this._zoomCenter.screenY() * this._zoom.y;

        xs = Math.abs(x - Graphics.width / 2 ) /  this._lastZoomSpeed   / 10;
        ys = Math.abs(y - 16 - Graphics.height / 2 ) / this._lastZoomSpeed  / 10;

        if (y - 16   > Graphics.height / 2)   $gameMap.scrollDown(ys);
        if (x < Graphics.width / 2)         $gameMap.scrollLeft(xs);
        if (x > Graphics.width / 2)         $gameMap.scrollRight(xs);
        if (y - 16  < Graphics.height / 2)   $gameMap.scrollUp(ys);
    }        
  ...
  Game_Event.prototype.update = function() {
    ...
    function Game_PointEvent (){
        this.initialize.apply(this, arguments);
    }

    Game_PointEvent.prototype = Object.create(Game_Event.prototype);
    Game_PointEvent.prototype.constructor = Game_PointEvent;
    Game_PointEvent.prototype.initialize = function(mapId, eventId) {
        Game_Character.prototype.initialize.call(this);
        this._mapId = mapId;
        this._eventId = eventId;
        this.refresh();
    };  
    Game_PointEvent.prototype.event = function() {};    
    Game_PointEvent.prototype.findProperPageIndex = function() {
    };
    Game_PointEvent.prototype.isOnBush = function() {};    
    Game_PointEvent.prototype.update =  function (){
        Game_Event.prototype.update.call(this);
    }
    Game_PointEvent.prototype.screenX = function() {
        var tw = $gameMap.tileWidth();
        
        return Math.round(this.scrolledX() * tw + tw / 2);
    };

    Game_PointEvent.prototype.screenY = function() {
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
  	...
  	if (command == "MapZoom") {
        ...
        $gameMap.setZoomCenter();
            ...
            if (args[2] === "0") ev = $gamePlayer;
  	  		if (ev){
                // $gameMap.setZoom($gameMap._destZoom.x, $gameMap._destZoom.y, Number(args[4]) - $gameMap._spdZoom);
  	  			$gameMap.setZoomCenter(ev , args[3], Number(args[4]));
                $gameMap.setDurationForView( Number(args[4]));                
            }
            ...
            $gameMap.setDurationForView( Number(args[3])); 
  	  		$gameMap.setZoomCenter(Number(args[1]), args[2], Number(args[3]));
